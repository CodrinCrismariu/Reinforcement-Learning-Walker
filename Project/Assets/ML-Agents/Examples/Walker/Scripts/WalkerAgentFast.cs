...
public class WalkerAgentFast : Agent
{
    ...
    // Internal counters
    private List<float> m_EpisodeTargetCounts = new List<float>();
    private int m_CurrentEpisodeTargetCount = 0;
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (!m_IsFirstEpisode)
        {
            m_EpisodeTargetCounts.Add(m_CurrentEpisodeTargetCount);
        }
        m_IsFirstEpisode = false;
        m_CurrentEpisodeTargetCount = 0; // Reset for the new episode starting now

        if (!m_HasLoggedMetrics && m_EpisodeTargetCounts.Count >= metricsAggregationEpisodes)
        {
            // Calculate Mean
            float sum = 0f;
            foreach (float val in m_EpisodeTargetCounts) sum += val;
            float mean = sum / m_EpisodeTargetCounts.Count;

            // Calculate Standard Error (SE = SD / sqrt(N))
            float sumSqDiff = 0f;
            foreach (float val in m_EpisodeTargetCounts) sumSqDiff += (val - mean) * (val - mean);
            float variance = sumSqDiff / m_EpisodeTargetCounts.Count;
            float stdDev = Mathf.Sqrt(variance);
            float stdErr = stdDev / Mathf.Sqrt(m_EpisodeTargetCounts.Count);

            Debug.Log($"[{agentID} Training Steps] Over last {m_EpisodeTargetCounts.Count} episodes: " +
                      $"Mean Number of Targets Collected = {mean:F4}, StdError = {stdErr:F4}");

            // Set flag to true so we never log again
            m_HasLoggedMetrics = true;

            // Clear window (optional, but good for memory)
            m_EpisodeTargetCounts.Clear();
        }
    ...
    {
        // Increment metric collected for this specific episode
        m_CurrentEpisodeTargetCount++;
