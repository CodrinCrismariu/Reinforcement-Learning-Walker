...
public class WalkerAgentCurriculum : Agent
{
    [Header("Walk Speed")]
    [Range(0.1f, 10)]
    [SerializeField]
    ...
    [Tooltip("Fixed penalty per step for knees touching ground")]
    public float maxKneeTouchPenalty = -5f;

    [Tooltip("Ratio of gravity to compensate (0.0 = no assist, 0.5 = half weight)")]
    public float currAssistForceRatio = 0f; // Controlled by Curriculum "curriculum_value"
    ...
    public override void OnEpisodeBegin()
    {
        ...
        // READ FROM CURRICULUM VALUE directly to Set Assist Force
        // Param "curriculum_value" is expected to be between 1.0 (Full assist) and -1.0 (No assist)
        // Or whatever the user defined in WalkerCurriculum.yaml
        float curriculumValue = m_ResetParams.GetWithDefault("curriculum_value", 0.0f);
        
        // Map curriculumValue to currAssistForceRatio.
        // If curriculumValue is just the force ratio directly, we use it directly.
        // Based on user request "choose it from the curriculum value", we blindly assign it.
        // If the user's YAML goes from 0.8 to -1.0, then assist force will vary accordingly.
        currAssistForceRatio = curriculumValue;

        if(curriculumValue < 0)
        {
            startingStage = LearningStage.Walking;
        }
        else
        {
            startingStage = LearningStage.Medium;
        }
        ...
        terminateOnBodyPartContact = false;
        ...
        switch (startingStage)
        {
            ...
            case LearningStage.Medium:
                ...
                thighL.localRotation = Quaternion.Euler(Random.Range(-90f, 90f), Random.Range(0.0f, 360.0f), 0);
                thighR.localRotation = Quaternion.Euler(Random.Range(-90f, 90f), Random.Range(0.0f, 360.0f), 0);
            ...
            default:
                terminateOnBodyPartContact = true;
    ...
    void FixedUpdate()
    {
        ...
        AddReward(matchSpeedReward * lookAtTargetReward * 10f);
        ...
        float heightReward = Mathf.Pow(Mathf.Clamp01(headHeight / 1.7f), 2);
